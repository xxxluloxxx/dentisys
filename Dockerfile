...
RUN --mount=type=cache,id=railway-maven-deps,target=/root/.m2 ./mvnw dependency:go-offline

# Ahora copiar el código fuente
COPY src src

# Construir la aplicación
RUN --mount=type=cache,id=railway-maven-build,target=/root/.m2 ./mvnw clean package -DskipTests
